...
def get_tts_wav(ref_wav_path, prompt_text, prompt_language, text, text_language, top_k=20,
                top_p=0.7, temperature=0.8, ref_free=False):
    ...
    with torch.no_grad():
        loss, acc = t2s_model.model.forward_old(
            all_phoneme_ids,
            all_phoneme_len,
...
def gen():
    # gpt_model_path = "/data1/xiepengyuan/.cache/huggingface/GPT-SoVITS/s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt"
    # SoVITS_model_path = "/data1/xiepengyuan/.cache/huggingface/GPT-SoVITS/s2G488k.pth"
    gpt_model_path = "/data1/xiepengyuan/workspace/audio/GPT-SoVITS/GPT_weights/base_mihoyo-e500.ckpt"
    ...
    os.environ["MASTER_PORT"] = str(20000)
    ...
    for batch in train_dataloader:
        # 在每个batch中处理数据
        loss, acc = t2s_model.model.forward_old(
            batch["phoneme_ids"].to("cuda"),
            batch["phoneme_ids_len"].to("cuda"),
            batch["semantic_ids"].to("cuda"),
            batch["semantic_ids_len"].to("cuda"),
            batch["bert_feature"].to("cuda")
        )
        pass
    # audio = get_tts_wav(
    ...
    # sf.write(output_wav_path, audio, 32000)
